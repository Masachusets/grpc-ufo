...
func createSighting(ctx context.Context, client ufo_v1.UFOServiceClient) (string, error) {
	...
	res, err := client.Create(ctx, &ufo_v1.CreateRequest{Info: info})
	if err != nil {
		return "", err
	}
...
func getSighting(ctx context.Context, client ufo_v1.UFOServiceClient, uuid string) (*ufo_v1.Sighting, error) {
	res, err := client.Get(ctx, &ufo_v1.GetRequest{Uuid: uuid})
	if err != nil {
		return nil, err
	}

	return res.Sighting, nil
}

// updateSighting обновляет наблюдение НЛО
func updateSighting(ctx context.Context, client ufo_v1.UFOServiceClient, uuid string) error {
...
func deleteSighting(ctx context.Context, client ufo_v1.UFOServiceClient, uuid string) error {
